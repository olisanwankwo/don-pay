<template>
  <div class="min-h-screen bg-gray-900 text-white relative">
    <div class="sidebar fixed left-0 w-64 h-full bg-gray-900 text-white transition-all duration-300 ease-in-out transform" :class="{ '-translate-x-full': !showSidebar }">
      ...
      <a href="/billhistory" class="block py-3 px-6 text-lg flex items-center hover:bg-gray-800 border-b border-gray-700">
    ...
    <div class="min-h-screen bg-gray-900 text-white">
      <div class="header flex justify-between items-center px-6 py-10">
        ...
        <h1 class="text-lg flex-2 text-center font-bold">Transaction History</h1>
          ...
          <table class="table-auto w-full">
            <thead>
              <tr class="bg-gray-800 text-gray-300">
                ...
                <th class="px-2 md:px-4 py-2">Description <span class="icon-arrow">&UpArrow;</span></th>
                <th class="px-2 md:px-4 py-2">Transaction Amount <span class="icon-arrow">&UpArrow;</span></th>
                <th class="px-2 md:px-4 py-2">Account Balance <span class="icon-arrow">&UpArrow;</span></th>
              </tr>
            </thead>
            <tbody class="text-gray-500">
              <tr v-for="(transaction, index) in combinedHistory" :key="index">
    ...
    const fetchTransactionHistory = async () => {
      try {
        ...
        const { data: transferHistory, error: transferError } = await supabase
          ...
          .select("*")
          .or(`sender_email.eq.${userInfo.value.email},recipient_email.eq.${userInfo.value.email}`);
        ...
        combinedHistory.value = [
          ...transferHistory.map(record => ({
            date: record.date,
            description: `Transfer - ${record.description}`,
